...
void generate_ip(unsigned int ip, char ip_addr[])
{
    ...
    bytes[2] = (ip >> 16) & 0xFF;
    bytes[3] = (ip >> 24) & 0xFF;
    snprintf(ip_addr,IP_ADDR_LEN_STR,"%d.%d.%d.%d", bytes[0], bytes[1], bytes[2], bytes[3]); 
}
...
int populate_packet_ds(const struct pcap_pkthdr *header, const u_char *packet, ETHER_Frame *custom_frame)
{
        ...
        if(ntohs(ethernet->ether_type) == ETHERTYPE_IP) 
        {
            ...
            if((int)ip->ip_p==UDP_PROTOCOL)
            {
                ...
                UDP_SEGMENT custom_segment;
                payload = (u_char *)(packet + SIZE_ETHERNET + size_ip + SIZE_UDP);
                custom_segment.source_port = ntohs(udp->th_sport);
                custom_segment.destination_port = ntohs(udp->th_dport);
                custom_segment.data = payload;
                custom_packet.udp_data = custom_segment;
                custom_packet.is_tcp = false;
                custom_packet.is_udp = true;    
                custom_packet.is_http = false;  
                custom_frame->packet_ip = custom_packet;
            ...
            if((int)ip->ip_p==TCP_PROTOCOL)
            {
                ...
                custom_packet.is_tcp = true;
                custom_packet.is_udp = false;
                custom_packet.is_ftp = false;    
                custom_frame->packet_ip = custom_packet;


                strcpy(custom_segment.protocole, "");

                if((custom_segment.source_port == 443) || (custom_segment.destination_port == 443))
                {
                    if(strstr((const char*)custom_segment.data, "HTTP/") == NULL)
                    {
                        strcpy(custom_segment.protocole, "https");
                    }

                }

                if((custom_segment.source_port == 21) || (custom_segment.destination_port == 21) || (custom_segment.destination_port == 20) || (custom_segment.destination_port == 20))
                {
                    strcpy(custom_segment.protocole, "ftp");
                    custom_packet.is_ftp = true;  
                }

                if((custom_segment.source_port == 80) || (custom_segment.destination_port == 80))
                {
                    if (strstr((const char*)custom_segment.data, "GET") || strstr((const char*)custom_segment.data, "POST") || strstr((const char*)custom_segment.data, "HTTP/"))     
                    {
                        strcpy(custom_segment.protocole, "http");
                    }
                }  
